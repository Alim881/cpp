...
bool Image::load(const std::string& filename) {
    ...
    pixels.assign(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(4)));

    std::vector<std::vector<png_byte>> row_data(height);
    for (int y = 0; y < height; ++y)
        row_data[y].resize(png_get_rowbytes(png, info));
...
void applyColorNoise(Image& img, float intensity) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            int noiseR = (rand() % 501 - 250) * noiseFactor;
            int noiseG = (rand() % 501 - 250) * noiseFactor;
            int noiseB = (rand() % 501 - 250) * noiseFactor;

            pixel[0] = std::min(255, std::max(0, static_cast<int>(pixel[0]) + noiseR));
            pixel[1] = std::min(255, std::max(0, static_cast<int>(pixel[1]) + noiseG));
            pixel[2] = std::min(255, std::max(0, static_cast<int>(pixel[2]) + noiseB));
            img.setPixel(x, y, pixel);
        }
    }
}
...
void applyGlitch(Image& img) {
    ...
    for (int y = 0; y < height; y += 10) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            auto pixel = img.getPixel(x, y);
            if (y % 20 == 0) pixel[0] = std::min(255, static_cast<int>(pixel[0]) + 50);
            else if (y % 15 == 0) pixel[1] = std::min(255, static_cast<int>(pixel[1]) + 50);
